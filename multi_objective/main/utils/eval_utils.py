...
class SillyWalks:
    ...
    def __init__(self, ref_mols, n_jobs=1):
        ...
        with Pool(n_jobs) as pool:
            bit_counts = [
                bc for bc in pool.imap(self.count_bits, ref_mols) if bc is not None
            ]
...
class ChemistryFilters:
    def __init__(
        ...
        self._property_filters = [
            lambda x: (self.ref_MW[0] - self.std_thresh * self.ref_MW[1])
            <= self.MolWt(x)
            <= (self.ref_MW[0] + self.std_thresh * self.ref_MW[1]),
            lambda x: (self.ref_LogP[0] - self.std_thresh * self.ref_LogP[1])
            <= self.MolLogP(x)
            <= (self.ref_LogP[0] + self.std_thresh * self.ref_LogP[1]),
        ...
        while (len(top_n) < n) and (i < len(mols)):
            ...
            if diverse_filters:
                ...
                if len(top_n_fps) > 0:
                    if not all(
                        [
                            DataStructs.TanimotoSimilarity(fp, div_fp)
                            <= self.diverse_thresh
                            for div_fp in top_n_fps
                        ]
                    ):
