...
from .mol_lm_utils import (clean_edits, clip_loss_for_edit,
                           evaluate_SMILES_list, get_description_list,
                           get_SMILES_list,
                           load_language_molecule_and_edit_models,
                           prepare_text_tokens)
...
class MolCLIP(nn.Module):
    def __init__(self):
        ...
        device = (
            torch.device("cuda:" + str(self.args.device))
        ...
        self.task2description = {
            "jnk3": "This molecule inhibits JNK3.",
    ...
    def load_modules(self, load_molmodel=False):
        (
            ...
            self.args, load_molmodel=load_molmodel
        )
    ...
    def get_text_repr(self, text):
        ...
        text_output = self.text_model(
            input_ids=text_tokens_ids, attention_mask=text_masks
        )
    ...
    def forward(self, molecule_data, text_prompt):
        ...
        molecule_repr = get_molecule_repr_MoleculeSTM(
            molecule_data,
            mol2latent=self.mol2latent,
            molecule_type="SMILES",
            MegaMolBART_wrapper=self.MegaMolBART_wrapper,
        )
    ...
    def check_edit(self, SMILES, text):
        ...
        latent_code_init, pad_mask_init = self.MegaMolBART_wrapper.smileslist2embedding(
            [SMILES]
        )  # [pad, B, d], [pad, B]
        first_and_second_SMILES_list.append(SMILES)

        regenerated_mols = self.MegaMolBART_wrapper.inverse_transform(
        ...
        for l2_lambda in self.l2_lambda_list:
            ...
            current_SMILES_list = [first_and_second_SMILES_list[0]] + [
                first_and_second_SMILES_list[1]
            ]
            if self.args.use_noise_for_init:
            ...
            for i in L:
                ...
                if self.args.normalize:
                    molecule_repr_generation = F.normalize(
                        molecule_repr_generation, dim=-1
                    )
                molecule_repr_MoleculeSTM = self.generation2MoleculeSTM(
            ...
            generated_mols = self.MegaMolBART_wrapper.inverse_transform(
                [latent], pad_mask.bool().cuda(), k=1, sanitize=True
            )
            ...
            try:
                ...
                pass
            result_SMILES_list_one_pair.append(
                [text] + current_SMILES_list + ["{}".format(l2_lambda)]
            )

            current_result_list, oracle_vals = evaluate_SMILES_list(
                current_SMILES_list, text
            )
            result_eval_list_one_pair.append(current_result_list)

        result_eval_list_one_pair = np.array(result_eval_list_one_pair)
        result_eval_list_one_pair = np.any(
            result_eval_list_one_pair, axis=0, keepdims=True
        )
    ...
    def parse_args(self):
        ...
        ########## for editing ##########
        parser.add_argument(
            "--use_noise_for_init", dest="use_noise_for_init", action="store_true"
        )
        parser.add_argument(
            "--no_noise_for_init", dest="use_noise_for_init", action="store_false"
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default="/scratch/ssd004/scratch/mskrt/huggingface_models/MoleculeSTM/megamolbart/models/megamolbart/checkpoints",
        )
        parser.add_argument(
            "--vocab_path",
            type=str,
            default="/h/mskrt/language_guided_genetic_algorithms/MoleculeSTM/bart_vocab.txt",
        )

        ########## for MoleculeSTM and generation projection ##########
        parser.add_argument(
            "--language_edit_model_dir",
            type=str,
            default="/scratch/ssd004/scratch/mskrt/huggingface_models/MoleculeSTM/demo/demo_checkpoints_SMILES",
        )
