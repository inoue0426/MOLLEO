...
from MoleculeSTM.cuchemcommon.workflow import (BaseGenerativeWorkflow,
                                               add_jitter)

from .decoder import DecodeSampler
from .megatron_bart import MegatronBART
from .tokenizer import MolEncTokenizer
from .util import (DEFAULT_CHEM_TOKEN_START, DEFAULT_D_MODEL,
                   DEFAULT_MAX_SEQ_LEN, DEFAULT_NUM_HEADS, DEFAULT_NUM_LAYERS,
                   DEFAULT_VOCAB_PATH, REGEX)
...
def _(embedding, radius, cnt, shape):
    if shape is not None:
        embedding = torch.reshape(embedding, (1, shape[0], shape[1])).to(
            embedding.device
        )
...
class MegaMolBART(BaseGenerativeWorkflow):
    ...
    def __init__(
        ...
        self.tokenizer = self.load_tokenizer(
            args.vocab_file, regex, default_chem_token_start
        )
    ...
    def load_tokenizer(self, tokenizer_vocab_path, regex, default_chem_token_start):
        ...
        tokenizer = MolEncTokenizer.from_vocab_file(
            tokenizer_vocab_path, regex, default_chem_token_start
        )
    ...
    def save_model(self, iteration, model, optimizer=None, lr_scheduler=None):
        ...
        if mpu.get_data_parallel_rank() == 0:
            ...
            # Save.
            checkpoint_name = megatron_checkpointing.get_checkpoint_name(
                args.save, iteration
            )
            print(
        ...
        if torch.distributed.get_rank() == 0:
            tracker_filename = megatron_checkpointing.get_checkpoint_tracker_filename(
                args.save
            )
    ...
    def smiles2embedding(self, smiles, pad_length=None):
        ...
        token_ids = (
            torch.tensor(
                self.tokenizer.convert_tokens_to_ids(tokens["original_tokens"])
            )
            .cuda()
            .T
        )
    ...
    def inverse_transform(self, embeddings, mem_pad_mask, k=1, sanitize=True):
        ...
        with torch.no_grad():
            for memory in embeddings:
                ...
                decode_fn = partial(
                    self.model._decode_fn,
                    mem_pad_mask=mem_pad_mask.type(torch.LongTensor).cuda(),
                    memory=memory,
                )

                mol_strs, _ = self.model.sampler.beam_decode(
                    decode_fn, batch_size=batch_size, device="cuda", k=k
                )
                ...
                for smiles in mol_strs:
                    if sanitize:
                        ...
                        if mol:
                            ...
                            logger.debug(
                                f"Sanitized SMILES {sanitized_smiles} added..."
                            )
                            break
    ...
    def interpolate_molecules(self, smiles1, smiles2, num_interp, tokenizer, k=1):
        ...
        embedding1, pad_mask1 = self.smiles2embedding(smiles1, pad_length=pad_length)

        embedding2, pad_mask2 = self.smiles2embedding(smiles2, pad_length=pad_length)
        ...
        interpolated_emb = torch.lerp(
            embedding1, embedding2, scale
        ).cuda()  # dims: batch, tokens, embedding
        ...
        generated_mols = self.inverse_transform(
            embeddings, combined_mask, k=k, sanitize=True
        )
    ...
    def find_similars_smiles_list(
        ...
        generated_mols = self.inverse_transform(
            neighboring_embeddings, pad_mask.bool().cuda(), k=1, sanitize=True
        )
        if force_unique:
    ...
    def find_similars_smiles(
        ...
        generated_mols, neighboring_embeddings, pad_mask = (
            self.find_similars_smiles_list(
                smiles,
        ...
        if force_unique:
            inv_transform_funct = partial(self.inverse_transform, mem_pad_mask=pad_mask)
            generated_df = self.compute_unique_smiles(
                generated_df, inv_transform_funct, scaled_radius=scaled_radius
            )
        return generated_df

    def interpolate_smiles(
        self, smiles: List, num_points: int = 10, scaled_radius=None, force_unique=False
    ):
        ...
        for idx in range(len(smiles) - 1):
            interpolated_mol, interpolated_embeddings, combined_mask, dims = (
                self.interpolate_molecules(
                    smiles[idx], smiles[idx + 1], num_points, self.tokenizer, k=k
                )
            )
            ...
            inv_transform_funct = partial(
                self.inverse_transform, mem_pad_mask=combined_mask
            )
            ...
            if force_unique:
                interp_df = self.compute_unique_smiles(
                    interp_df, inv_transform_funct, scaled_radius=scaled_radius
                )
