...
from MoleculeSTM.downstream_molecule_edit_utils import (
    clip_loss_for_edit, evaluate_SMILES_list, get_description_list,
    get_SMILES_list, load_language_molecule_and_edit_models)
...
def check_edit(SMILES, text):
    ...
    latent_code_init, pad_mask_init = MegaMolBART_wrapper.smileslist2embedding(
        [SMILES]
    )  # [pad, B, d], [pad, B]
    first_and_second_SMILES_list.append(SMILES)

    regenerated_mols = MegaMolBART_wrapper.inverse_transform(
    ...
    for l2_lambda in l2_lambda_list:
        ...
        for i in L:
            ...
            optimizer.step()
        print(
            "clip loss: {:.5f}\tL2 loss: {:.5f}".format(
                clip_loss_.item(), l2_loss_.item()
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_SMILES_file",
        type=str,
        default="/scratch/ssd004/scratch/mskrt/datasets/Editing_data/single_multi_property_SMILES.txt",
    )
    ...
    ########## for MoleculeSTM ##########
    parser.add_argument(
        "--MoleculeSTM_model_dir", type=str, default="../../pretrained_model_Raw"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--language_edit_model_dir", type=str, default="edit_temp/EBM_NCE"
    )
    ...
    ) = load_language_molecule_and_edit_models(args)
    device = (
    ...
    for description in description_list:
        ...
        result_acc_list = 100.0 * result_acc_list / len(source_SMILES_list)
