...
from MoleculeSTM.datasets import (PubChemSTM_Datasets_Graph,
...
def save_model(save_best, epoch=None):
    if args.output_model_dir is not None:
        ...
        saved_file_path = os.path.join(
            args.output_model_dir, "text_{}".format(model_file)
        )
        torch.save(text_model.state_dict(), saved_file_path)

        saved_file_path = os.path.join(
            args.output_model_dir, "molecule_{}".format(model_file)
        )
        torch.save(molecule_model.state_dict(), saved_file_path)

        saved_file_path = os.path.join(
            args.output_model_dir, "text2latent_{}".format(model_file)
        )
        torch.save(text2latent.state_dict(), saved_file_path)

        saved_file_path = os.path.join(
            args.output_model_dir, "mol2latent_{}".format(model_file)
        )
...
def train(
    ...
    print(
        "CL Loss: {:.5f}\tCL Acc: {:.5f}\tTime: {:.5f}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--representation_frozen", dest="representation_frozen", action="store_true"
    )
    parser.add_argument(
        "--no_representation_frozen", dest="representation_frozen", action="store_false"
    )
    ...
    ########## for 2D GNN ##########
    parser.add_argument(
        "--pretrain_gnn_mode", type=str, default="GraphMVP_G", choices=["GraphMVP_G"]
    )
    ...
    if args.representation_frozen:
        ...
        model_param_group = [
            {
                "params": text2latent.parameters(),
                "lr": args.text_lr * args.text_lr_scale,
            },
