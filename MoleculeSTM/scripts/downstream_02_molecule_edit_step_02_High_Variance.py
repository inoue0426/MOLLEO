...
def check_edit(SMILES, text):
    ...
    for alpha in alpha_list:
        ...
        latent = latent_code_init + alpha * direction / len(latent_code_init)
...
if __name__ == "__main__":
    ...
    # calculate variance
    var_array = np.var(mol_repr, axis=0)
    assert len(var_array) == mol_repr.shape[1]
    var_index = np.argsort(var_array)
    sampled_direction_idx = var_index[0]
    direction = torch.zeros((mol_repr.shape[1]))
    direction[sampled_direction_idx] = 1
