...
from MoleculeSTM.datasets import (ZINC250K_Dataset_Graph,
                                  ZINC250K_Dataset_SMILES)
from MoleculeSTM.downstream_molecule_edit_utils import load_molecule_models
from MoleculeSTM.models import MLP
from MoleculeSTM.utils import freeze_network, get_molecule_repr_MoleculeSTM
...
def do_CL(X, Y, args):
    ...
    if args.SSL_loss == "EBM_NCE":
        ...
        SSL_loss = (loss_pos + args.CL_neg_samples * loss_neg) / (
            1 + args.CL_neg_samples
        )
        ...
        SSL_acc = pred.eq(labels).sum().detach().cpu().item() * 1.0 / B
...
def get_molecule_repr_generation(
    molecule_data, molecule_model, molecule_type="MegaMolBART", MegaMolBART_wrapper=None
):
    if molecule_type == "MegaMolBART":
        embedding, pad_mask = MegaMolBART_wrapper.smileslist2embedding_model_given(
            molecule_model, molecule_data
        )  # [pad, B, d], [pad, B]
...
def save_model(save_best, epoch=None):
    if args.output_model_dir is not None:
        ...
        saved_file_path = os.path.join(
            args.output_model_dir, "generation2MoleculeSTM_{}".format(model_file)
        )
        torch.save(generation2MoleculeSTM.state_dict(), saved_file_path)

        saved_file_path = os.path.join(
            args.output_model_dir, "MoleculeSTM2generation_{}".format(model_file)
        )
...
def train(epoch):
    ...
    for batch in L:
        ...
        if args.MoleculeSTM_molecule_type == "SMILES":
            molecule_repr_MoleculeSTM = get_molecule_repr_MoleculeSTM(
                SMILES_list,
                molecule_model=molecule_model_MoleculeSTM,
                mol2latent=mol2latent_MoleculeSTM,
                molecule_type=args.MoleculeSTM_molecule_type,
                MegaMolBART_wrapper=MegaMolBART_wrapper,
            )
            molecule_repr_MoleculeSTM2generation = MoleculeSTM2generation(
                molecule_repr_MoleculeSTM
            )
        ...
        if args.generation_model == "MegaMolBART":
            molecule_repr_generation = get_molecule_repr_generation(
                ...
                molecule_type="MegaMolBART",
                MegaMolBART_wrapper=MegaMolBART_wrapper,
            )
        else:  # for HierVAE
            hiervae_data_list = MolGraph.tensorize(SMILES_list, vocab, avocab)
            molecule_repr_generation = molecule_model_generation.forward_MoleculeSTM(
                hiervae_data_list
            )
        molecule_repr_generation2MoleculeSTM = generation2MoleculeSTM(
            molecule_repr_generation
        )

        loss_01, acc_01 = do_CL(
            molecule_repr_generation, molecule_repr_MoleculeSTM2generation, args
        )
        loss_02, acc_02 = do_CL(
            molecule_repr_MoleculeSTM, molecule_repr_generation2MoleculeSTM, args
        )
    ...
    print(
        "SSL Loss: {:.5f}\tSSL Acc: {:.5f}\tTime: {:.5f}".format(
            accum_loss, accum_acc, time.time() - start_time
        )
    )
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataspace_path", type=str, default="/scratch/ssd004/scratch/mskrt/datasets/"
    )
    parser.add_argument("--dataset", type=str, default="ZINC250K")
    parser.add_argument(
        "--MoleculeSTM_molecule_type",
        type=str,
        default="SMILES",
    ...
    ########## for generation ##########
    parser.add_argument(
        "--generation_model", type=str, default="MegaMolBART", choices=["MegaMolBART"]
    )

    ######### for MegaMolBART ##########
    parser.add_argument(
        "--MegaMolBART_generation_model_dir",
        type=str,
    ...
    parser.add_argument(
        "--MoleculeSTM_lr", type=float, default=1e-2
    )  # optimal: 1e-2 or 1e-3
    parser.add_argument("--T", type=float, default=0.1)
    parser.add_argument(
        "--SSL_loss", type=str, default="RR", choices=["EBM_NCE", "InfoNCE", "RR"]
    )
    ...
    generation2MoleculeSTM = MLP(
        molecule_dim_generation, [molecule_dim_MoleculeSTM, molecule_dim_MoleculeSTM]
    ).to(device)
    MoleculeSTM2generation = MLP(
        molecule_dim_MoleculeSTM, [molecule_dim_generation, molecule_dim_generation]
    ).to(device)
