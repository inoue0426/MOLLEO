...
from MoleculeSTM.downstream_molecule_edit_utils import (evaluate_SMILES_list,
...
def get_molecule_repr_generation(
    molecule_data, molecule_model, molecule_type="SMILES", MegaMolBART_wrapper=None
):
...
def check_edit(SMILES, text):
    ...
    for alpha in alpha_list:
        ...
        latent = latent_code_init + alpha * latent_repr_pca / len(latent_code_init)
        ...
        result_SMILES_list_one_pair.append(
            [text] + current_SMILES_list + ["{}".format(alpha)]
        )

        print("current_SMILES_list", current_SMILES_list)
        print("result_SMILES_list_one_pair", result_SMILES_list_one_pair)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_SMILES_file",
        type=str,
        default="../data/Editing_data/single_multi_property_SMILES.txt",
    )
    ...
    MegaMolBART_wrapper = MegaMolBART(
        input_dir=args.generation_model_dir, output_dir=None
    )
    ...
    mol_repr = get_molecule_repr_generation(
        source_SMILES_list,
        molecule_model=molecule_model_generation,
        molecule_type="SMILES",
        MegaMolBART_wrapper=MegaMolBART_wrapper,
    )
    mol_repr = mol_repr.detach().cpu().numpy()
    latent_repr_pca = PCA(n_components=1).fit_transform(mol_repr.transpose())[:, 0]
    latent_repr_pca = torch.tensor(latent_repr_pca).to(device)
    latent_repr_pca = F.normalize(latent_repr_pca, dim=-1)
