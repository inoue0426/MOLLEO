...
from MoleculeSTM.datasets import (DrugBank_Datasets_Graph_retrieval,
                                  DrugBank_Datasets_SMILES_retrieval,
...
def extract_retrieval_representation(retrieval_dataloader):
    ...
    for step, batch in enumerate(L):
        ...
        try:
            ...
            description_output = text_model(
                input_ids=description_tokens_ids, attention_mask=description_masks
            )
            ...
            continue
        retrieval_description_representation_list.append(
            description_repr.detach().cpu().numpy()
        )
        retrieval_molecule_repr_list.append(molecule_repr.detach().cpu().numpy())

    retrieval_description_representation_array = np.concatenate(
        retrieval_description_representation_list
    )
    retrieval_molecule_representation_array = np.concatenate(
        retrieval_molecule_repr_list
    )

    return (
        retrieval_description_representation_array,
    ...
    for batch in L:
        ...
        if test_mode == "given_text":
            if args.molecule_type == "SMILES":
                ...
                neg_molecule_repr = [
                    get_molecule_repr_MoleculeSTM(
                        neg_molecule_data[idx].to(device),
                        mol2latent=None,
                        molecule_type="Graph",
                        molecule_model=molecule_model,
                    )
            ...
            similarity_array = get_similarity_array(
                text_repr, retrieval_description_representation_dataloader
            )
            ...
            for batch_i in range(batch_size):
                ...
                retrieved_text_repr_list.append(
                    retrieval_molecule_representation_array[optimal_index]
                )
            ...
            for T_idx, T in enumerate(args.T_list):
                _, _, acc = do_CL_eval(
                    retrieved_text_repr, molecule_repr, neg_molecule_repr[: T - 1], args
                )
            ...
            similarity_array = get_similarity_array(
                molecule_repr, retrieval_molecule_representation_dataloader
            )
            batch_size = similarity_array.shape[0]
            ...
            for batch_i in range(batch_size):
                ...
                sorted_index = np.argsort(temp_similarity_array)[::-1]
                optimal_index = sorted_index[0]
                retrieved_mol_repr_list.append(
                    retrieval_description_representation_array[optimal_index]
                )
            ...
            for T_idx, T in enumerate(args.T_list):
                _, _, acc = do_CL_eval(
                    retrieved_mol_repr, text_repr, neg_text_repr[: T - 1], args
                )
                accum_acc_list[T_idx] += acc
        else:
            raise Exception
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--text_type", type=str, default="SciBERT", choices=["SciBERT", "BioBERT"]
    )
    parser.add_argument("--load_latent_projector", type=int, default=1)
    parser.add_argument(
        "--model_loading_mode",
        type=str,
        default="load_from_latest",
        choices=["load_from_latest", "load_mode_0", "load_mode_1"],
    )
    parser.add_argument(
        "--training_mode", type=str, default="zero_shot", choices=["zero_shot"]
    )
    ...
    parser.add_argument(
        "--task",
        type=str,
        default="molecule_description",
        choices=[
            "molecule_description",
            "molecule_description_Raw",
            "molecule_description_removed_PubChem",
            "molecule_description_removed_PubChem_Raw",
            "molecule_pharmacodynamics",
            "molecule_pharmacodynamics_Raw",
            "molecule_pharmacodynamics_removed_PubChem",
            "molecule_pharmacodynamics_removed_PubChem_Raw",
        ],
    )
    parser.add_argument(
        "--test_mode",
    ...
    parser.add_argument(
        "--SSL_loss", type=str, default="EBM_NCE", choices=["EBM_NCE", "InfoNCE"]
    )
    ...
    ########## for molecule model ##########
    parser.add_argument(
        "--molecule_type", type=str, default="SMILES", choices=["SMILES", "Graph"]
    )

    ########## for MegaMolBART ##########
    parser.add_argument(
        "--vocab_path", type=str, default="../MoleculeSTM/bart_vocab.txt"
    )
    ...
    if args.text_type == "SciBERT":
        pretrained_SciBERT_folder = os.path.join(
            args.dataspace_path, "pretrained_SciBERT"
        )
    ...
    if args.molecule_type == "SMILES":
        if args.model_loading_mode == "load_from_latest":
            ...
            MegaMolBART_wrapper = MegaMolBART(
                vocab_path=args.vocab_path, input_dir=None, output_dir=None
            )
            ...
            MegaMolBART_wrapper = MegaMolBART(
                ...
                input_dir="../data/pretrained_MegaMolBART/checkpoints",
                output_dir=None,
            )
            molecule_model = MegaMolBART_wrapper.model
        ...
        if args.model_loading_mode == "load_from_latest":
            ...
            molecule_model.from_pretrained(
                "../data/pretrained_GraphMVP/GraphMVP_G/model.pth"
            )
    ...
    full_dataloader = dataloader_class(
        full_dataset,
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=args.num_workers,
    )  # The program will get blcoked with none-zero num_workers
    retrieval_dataloader = dataloader_class(
        retrieval_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
    ...
    retrieval_datapath = "{}/{}_{}".format(
        args.retrieval_folder, args.molecule_type, args.task
    )
    if os.path.exists(retrieval_datapath + ".npz"):
        data = np.load(retrieval_datapath + ".npz")
        retrieval_description_representation_array = data[
            "retrieval_description_representation_array"
        ]
        retrieval_molecule_representation_array = data[
            "retrieval_molecule_representation_array"
        ]
    else:
        (
            retrieval_description_representation_array,
            retrieval_molecule_representation_array,
        ) = extract_retrieval_representation(retrieval_dataloader)
        np.savez(
            retrieval_datapath,
            retrieval_description_representation_array=retrieval_description_representation_array,
            retrieval_molecule_representation_array=retrieval_molecule_representation_array,
        )
    retrieval_description_representation_dataset = RetrievalDataset(
        retrieval_description_representation_array
    )
    retrieval_description_representation_dataloader = DataLoader(
        retrieval_description_representation_dataset,
        batch_size=512,
        shuffle=False,
        num_workers=args.num_workers,
    )
    retrieval_molecule_representation_dataset = RetrievalDataset(
        retrieval_molecule_representation_array
    )
    retrieval_molecule_representation_dataloader = DataLoader(
        retrieval_molecule_representation_dataset,
        batch_size=512,
        shuffle=False,
        num_workers=args.num_workers,
    )
