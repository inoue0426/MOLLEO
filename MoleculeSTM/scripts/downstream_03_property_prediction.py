...
from MoleculeSTM.datasets import (MoleculeNetGraphDataset,
                                  MoleculeNetSMILESDataset)
...
def train_classification(model, device, loader, optimizer):
    ...
    for step, batch in enumerate(L):
        if args.molecule_type == "SMILES":
            ...
            molecule_repr = get_molecule_repr_MoleculeSTM(
                batch, mol2latent=None, molecule_type="Graph", molecule_model=model
            )
            pred = linear_model(molecule_repr)
            pred = pred.float()
...
if __name__ == "__main__":
    ...
    smiles_list = pd.read_csv(dataset_folder + "/processed/smiles.csv", header=None)[
        0
    ].tolist()
    ...
    val_loader = dataloader_class(
        valid_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
    )
    test_loader = dataloader_class(
        test_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
    ...
    if args.molecule_type == "SMILES":
        ...
        if args.input_model_path is not None:
            print(
                "Update MegaMolBART with pretrained MoleculeSTM. Loading from {}...".format(
        ...
        if args.input_model_path is not None:
            if "GraphMVP" in args.input_model_path:
                print(
                    "Start from pretrained model (GraphMVP) in {}.".format(
                        args.input_model_path
                    )
                )
    ...
    optimizer = optim.Adam(
        model_param_group, lr=args.lr, weight_decay=args.weight_decay
    )
    ...
    if task_mode == "classification":
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.eval_train:
                train_roc, train_acc, train_target, train_pred = eval_func(
                    model, device, train_loader
                )
            else:
                train_roc = train_acc = 0
            val_roc, val_acc, val_target, val_pred = eval_func(
                model, device, val_loader
            )
            test_roc, test_acc, test_target, test_pred = eval_func(
            ...
            if val_roc > best_val_roc:
                ...
                if args.output_model_dir is not None:
                    ...
                    output_model_path = os.path.join(
                        args.output_model_dir, "{}_model_best.pth".format(args.dataset)
                    )
                    ...
                    filename = os.path.join(
                        ...
                        "{}_evaluation_best.pth".format(args.dataset),
                    )
                    np.savez(
                        filename,
        ...
        for metric in metric_list:
            print(
                "Best (RMSE), {} train: {:.6f}\tval: {:.6f}\ttest: {:.6f}".format(
                    metric,
                    train_result_list[best_val_idx][metric],
                    val_result_list[best_val_idx][metric],
    ...
    if args.output_model_dir is not None:
        output_model_path = os.path.join(
            args.output_model_dir, "{}_model_final.pth".format(args.dataset)
        )
        saved_model_dict = {"model": model.state_dict()}
        torch.save(saved_model_dict, output_model_path)
