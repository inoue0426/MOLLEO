...
from transformers import (AutoModel, AutoTokenizer, BertForPreTraining,
                          BertTokenizer)
    ...
    for batch in L:
        ...
        if test_mode == "given_text":
            if args.molecule_type == "SMILES":
                neg_molecule_repr = [
                    get_text_and_SMILES_repr(neg_molecule_data[idx])
            ...
            neg_text_repr = [
                get_text_and_SMILES_repr(neg_text[idx]) for idx in range(T_max)
            ]
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        model.load_state_dict(
            torch.load(
                "../data/pretrained_KV-PLM/ckpt_ret01.pt",
